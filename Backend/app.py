...
# # file_path = 'C:/Users/Deepak Singh/Desktop/SIH-2/TitleVerifier/Backend/dataset/output_file1.xlsx'
...
# #     print("hi from the /check-title route")
...
def check_title_similarity():
    ...
    if not result.empty:
        ...
        return jsonify({
            "message": "The input title is too similar to existing titles and cannot be accepted.",
            "top_similar_titles": top_similar_titles[['Title Name']].to_dict(orient='records'),
            "acceptable_unique_titles": acceptable_unique_titles[['Title Name']].head(5).to_dict(orient='records')
        }), 202
    else:
        return jsonify({
            "message": "The input title does not have significant similarity to existing titles and can be accepted.",
            "acceptable_unique_titles": result[['Title Name']].head(5).to_dict(orient='records')
        }), 200
